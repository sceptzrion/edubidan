...
import { Menu, Bell, ChevronDown, User, Home, LogOut } from "lucide-react";
...
export function LecturerTopbar({ sidebarOpen, setSidebarOpen }: LecturerTopbarProps) {
  const router = useRouter();
  const [showAccount, setShowAccount] = useState(false);
  const [showNotif, setShowNotif] = useState(false);

  const notifications = [
    { id: 1, title: "Tugas Terkumpul", desc: "12 Mahasiswa telah mengumpulkan kuis ANC Terpadu.", time: "Baru saja", read: false },
    { id: 2, title: "Sistem", desc: "Pembaruan fitur rekap nilai otomatis telah aktif.", time: "1 hari lalu", read: true },
  ];
    ...
    <header className="h-16 md:h-18 shrink-0 border-b border-border bg-card/80 backdrop-blur-md flex items-center justify-between px-4 md:px-8 z-30 sticky top-0">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="lg:hidden flex items-center gap-2">
           ...
           <span className="text-[10px] font-extrabold bg-primary/10 text-primary px-2 py-0.5 rounded-full uppercase tracking-wider hidden sm:block">Dosen</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 md:gap-4">
          ...
          {showNotif && (
            <div className="absolute right-0 lg:right-auto lg:left-1/2 lg:-translate-x-1/2 top-[calc(100%+8px)] w-72 sm:w-80 bg-card border border-border rounded-2xl shadow-xl z-50 animate-in slide-in-from-top-2 duration-200">
              <div className="px-4 py-3 sm:px-5 sm:py-4 border-b border-border flex items-center justify-between bg-muted/30 rounded-t-2xl">
                <span className="text-sm sm:text-base font-extrabold text-foreground">Notifikasi Dosen</span>
              </div>
        ...
        <div className="relative">
          ...
          >
            ...
            <span className="text-xs md:text-sm font-extrabold text-foreground hidden md:block">Dr. Rina</span>
          ...
          {showAccount && (
            <div className="absolute right-0 top-[calc(100%+8px)] w-48 sm:w-56 bg-card border border-border rounded-2xl shadow-xl py-1.5 sm:py-2 z-100 animate-in slide-in-from-top-2 duration-200">
              <div className="px-3 py-2 sm:px-4 sm:py-3 border-b border-border mb-1 bg-muted/30">
                <p className="text-xs sm:text-sm font-extrabold text-foreground">Dr. Rina Hartati</p>
                <p className="text-[10px] sm:text-xs font-medium text-muted-foreground mt-0.5 truncate">rina.h@edubidan.id</p>
              </div>
              
              <button onClick={() => { router.push("/dashboard/lecturer/settings"); setShowAccount(false); }} className="w-full flex items-center gap-2.5 sm:gap-3 px-3 py-2 sm:px-4 sm:py-2.5 text-xs sm:text-sm font-bold text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
