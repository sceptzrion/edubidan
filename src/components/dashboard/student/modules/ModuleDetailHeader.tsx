    ...
    <div className="bg-card rounded-3xl border border-border overflow-hidden mb-8 shadow-sm">
      ...
      <div className="relative aspect-21/9 sm:aspect-16/5 bg-linear-to-br from-primary/30 to-teal-300 overflow-hidden">
        ...
        <div className="absolute top-4 right-4 sm:top-6 sm:right-6">
          {/* REVISI MOBILE: px dan text dikecilkan untuk HP */}
          <span className="bg-background/40 backdrop-blur-sm text-white px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl text-[10px] sm:text-xs font-bold shadow-lg border border-white/20">
            {info.category}
          </span>
        </div>

        <div className="absolute bottom-5 left-4 sm:bottom-10 sm:left-8 right-4 sm:right-8">
          {/* REVISI MOBILE: Ukuran teks persentase dan judul disesuaikan */}
          <p className="text-[10px] sm:text-sm font-extrabold text-primary mb-1 sm:mb-1.5 drop-shadow-sm">{info.progress}% Selesai</p>
          <h1 className="text-xl sm:text-4xl text-white font-extrabold drop-shadow-md leading-tight max-w-3xl">{info.title}</h1>
        </div>

        {/* REVISI MOBILE: Tinggi progress bar sedikit ditipiskan di HP */}
        <div className="absolute bottom-0 left-0 right-0 h-1 sm:h-1.5 bg-white/20">
           <div className="h-full bg-primary transition-all duration-1000 ease-out" style={{ width: `${info.progress}%` }} />
        </div>
      </div>

      {/* Detail Informasi */}
      <div className="p-5 sm:p-8">
        {/* REVISI MOBILE: Teks deskripsi jadi text-xs di HP */}
        <p className="text-xs sm:text-base text-muted-foreground font-medium leading-relaxed mb-6 sm:mb-8 max-w-4xl">
          {info.description}
        </p>

        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
          {/* REVISI MOBILE: Padding, ukuran icon, dan font dikecilkan untuk HP */}
          <div className="flex items-center gap-2.5 sm:gap-3 p-3 sm:p-4 rounded-2xl bg-muted/50 border border-border/50">
            ...
            <div className="min-w-0">
              <p className="text-[10px] sm:text-xs font-semibold text-muted-foreground mb-0.5 truncate">Estimasi Waktu</p>
          ...
          <div className="flex items-center gap-2.5 sm:gap-3 p-3 sm:p-4 rounded-2xl bg-muted/50 border border-border/50">
            ...
            <div className="min-w-0">
              <p className="text-[10px] sm:text-xs font-semibold text-muted-foreground mb-0.5 truncate">Jumlah Konten</p>
              <p className="text-xs sm:text-sm font-extrabold text-foreground truncate">3 Materi, 2 Kuis</p>
            </div>
          </div>
          ...
          <div className="col-span-2 md:col-span-1 flex items-center gap-2.5 sm:gap-3 p-3 sm:p-4 rounded-2xl bg-muted/50 border border-border/50">
            ...
            <div className="min-w-0">
              <p className="text-[10px] sm:text-xs font-semibold text-muted-foreground mb-0.5 truncate">Instruktur</p>
              <p className="text-xs sm:text-sm font-extrabold text-foreground truncate">{info.instructor.name}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border p-4 sm:p-6 bg-card">
          <div className="flex items-center gap-2 mb-3 sm:mb-4">
            ...
            <p className="text-sm sm:text-base font-extrabold text-foreground">Tujuan Pembelajaran Modul</p>
          </div>
          <ul className="space-y-2.5 sm:space-y-3">
            {info.objectives.map((o, i) => (
              <li key={i} className="flex items-start gap-2.5 sm:gap-3 text-xs sm:text-sm font-medium text-muted-foreground">
